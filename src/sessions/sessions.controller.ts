...
export class SessionsController {
  ...
  create(@Body() dto: CreateSessionDto) {
    // 暂时不传 user
    return this.service.create(dto, 'mock-user-id'); // 你可以用 mock 数据模拟 user._id
  }

  @Get()
  findAll() {
    return this.service.findAllByUser('mock-user-id');
  }

  @Get(':id')
  findOne(@Param('id') id: string) {
    return this.service.findOneIfOwnedByUser(id, 'mock-user-id');
  }
}
