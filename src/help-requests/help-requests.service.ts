...
import { 
  ...
  export class HelpRequestsService {
     ...
     * 创建新的帮助请求
     */
    async create(userId: string, createHelpRequestDto: CreateHelpRequestDto): Promise<HelpRequestDocument> {
      // 验证用户ID
      ...
      // 查找支持人员并发送通知
      const supportStaff = await this.usersService.findAll(1, 1, [UserRole.SUPPORT]);
    ...
    async findAllForUser(
      ...
      const [requests, total] = await Promise.all([
        this.helpRequestModel.find(query)
          .populate('assignedTo', 'username email')
    ...
    async findAll(
      ...
      const { status, priority, category, assignedTo, page = 1, limit = 10 } = queryDto;
    ...
    async update(
      ...
      if (userId && helpRequest.user.toString() !== userId && !helpRequest.assignedTo?.toString() !== userId) {
        throw new ForbiddenException('You do not have permission to update this help request');
      }
      
      // 更新字段
      if (updateHelpRequestDto.title !== undefined) helpRequest.title = updateHelpRequestDto.title;
      if (updateHelpRequestDto.issue !== undefined) helpRequest.issue = updateHelpRequestDto.issue;
      if (updateHelpRequestDto.category !== undefined) helpRequest.category = updateHelpRequestDto.category;
      if (updateHelpRequestDto.priority !== undefined) helpRequest.priority = updateHelpRequestDto.priority;
      ...
      if (updateHelpRequestDto.status !== undefined) {
        ...
        if (updateHelpRequestDto.status === HelpRequestStatus.RESOLVED || 
    ...
    async addResponse(
      ...
      const isSupport = responder.roles.some(r => 
        r === UserRole.SUPPORT || r === UserRole.ADMIN
      );
    ...
    async assignToUser(
      ...
      const isSupport = assignee.roles.some(r => 
        r === UserRole.SUPPORT || r === UserRole.ADMIN
      );
      
      if (!isSupport) {
        throw new BadRequestException('Assignee must be a support staff or admin');
      }
    ...
    async getStats(): Promise<{
      ...
      const [
        ...
        this.helpRequestModel.countDocuments({ status: HelpRequestStatus.IN_PROGRESS }),
        this.helpRequestModel.countDocuments({ status: HelpRequestStatus.RESOLVED }),
        this.helpRequestModel.countDocuments({ status: HelpRequestStatus.CLOSED }),
