...
export class MentorsService {
  ...
  async findByUserId(userId: string): Promise<MentorDocument> {
    ...
    if (!mentor) {
      throw new NotFoundException(
        `Mentor profile for user with ID ${userId} not found`,
      );
    }
  ...
  async update(
    ...
    // 更新字段
    if (updateMentorDto.expertise !== undefined)
    ...
    if (updateMentorDto.profileImageUrl !== undefined)
      mentor.profileImageUrl = updateMentorDto.profileImageUrl;
    if (updateMentorDto.education !== undefined)
      mentor.education = updateMentorDto.education;
    if (updateMentorDto.experience !== undefined)
      mentor.experience = updateMentorDto.experience;
    if (updateMentorDto.languages !== undefined)
      mentor.languages = updateMentorDto.languages;
    if (updateMentorDto.timeZone !== undefined)
      mentor.timeZone = updateMentorDto.timeZone;
  ...
  async search(searchDto: MentorSearchDto): Promise<{
    ...
    if (searchTerm) {
      ...
      query.$or = [{ bio: regex }, { expertise: regex }];
    }
  ...
  async updateAvailability(
    mentorId: string,
    availabilityDtos: AvailabilityDto[],
  ...
  async updateRating(
    ...
    // 更新累计评分和评分次数
    mentor.rating =
      (mentor.rating * mentor.ratingCount + newRating) /
