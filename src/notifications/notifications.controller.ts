...
export class NotificationsController {
  ...
  async findAll(@CurrentUser() user, @Query() queryDto: NotificationQueryDto) {
  ...
  @ApiOperation({
    summary: 'Send system notification to multiple users (Admin only)',
  })
  @ApiResponse({
    status: 201,
  ...
  async sendSystemNotification(
    ...
    const notifications =
      await this.notificationsService.createNotificationForMultipleUsers(
        userIds,
        message,
        title,
      );
